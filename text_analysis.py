...
def most_common_emojis(messages, n=5):
    def emojis_per_mess(message): return "".join(
...
def avg_message_wc(messages):
    return sum(map(lambda mess: len(text_only(mess).split(" ")) - 1, messages)) / len(messages)
...
def get_datetime(message):
    ...
    return datetime.datetime(day=date[0], month=date[1], year=date[2] + 2000, hour=time[0], minute=time[1], second=time[2])
